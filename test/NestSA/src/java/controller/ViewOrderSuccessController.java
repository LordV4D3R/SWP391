...
public class ViewOrderSuccessController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<OrderDTO> order = dao.viewOrderSuccess();
            if (order.size() > 0) {
                request.setAttribute("VIEW_ORDER_SUCCESS_VER_FULL", order);
                url = SUCCESS;
            } else if (order.isEmpty()) {
                request.setAttribute("VIEW_ORDER_SUCCESS_VER_FULL", "Hiện tại không có đơn hàng nào đang giao cả");
                url = SUCCESS;
            }
        } catch (Exception e) {
        } finally {
